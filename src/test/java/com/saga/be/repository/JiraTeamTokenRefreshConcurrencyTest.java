...
/**
 * Two {@link JiraTeamTokenService} instances share one DB (two Railway replicas). Both see an
 * expired token, both perform refresh HTTP, and persist uses {@code lockById} so the loser adopts
 * the winner's tokens instead of overwriting them.
 */
@DataJpaTest
...
class JiraTeamTokenRefreshConcurrencyTest {
	...
	@Test
	void twoReplicas_secondPersistAdoptsWinnerTokens() throws Exception {
		...
		String aad = TokenEncryptor.aad("pending", "JIRA", connected.getId().toString());
		row.setEncryptedAccessToken(encryptor.encrypt("access-stale", aad));
		...
		when(oauth.refresh(any())).thenAnswer(inv -> {
			...
			bothRefreshing.await(10, TimeUnit.SECONDS);
		...
		try {
			...
			String tokenA = first.get(20, TimeUnit.SECONDS);
			String tokenB = second.get(20, TimeUnit.SECONDS);
			...
			verify(oauth, times(2)).refresh("refresh-live");
		} finally {
			pool.shutdownNow();
		}
	}
	...
	private static UserAccount userAccount() {
		...
		account.setEmail("jira-refresh-" + UUID.randomUUID() + "@fpt.edu.vn");
