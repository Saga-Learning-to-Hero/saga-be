...
public class AiAnalysisSubmissionService {
	...
	public Submission submit(UUID userId, UUID projectId, UUID gitCommitId) {
		...
		String revision = commit.getShaHash() == null || commit.getShaHash().isBlank() ? commit.getId().toString() : commit.getShaHash(); String evidenceHash = hashEvidence(draft); String idempotency = AiHashes.sha256(String.join("|", projectId.toString(), AiArtifactType.COMMIT.name(), gitCommitId.toString(), revision, AiAnalysisType.COMMIT_INTELLIGENCE.name(), evidenceHash, POLICY_VERSION, PROMPT_VERSION, SCHEMA_VERSION, provider.providerConfigHash()));
		return Objects.requireNonNull(transactions.execute(status -> persist(commit, draft, evidenceHash, idempotency, revision, provider)));
	}
	private Submission persist(GitCommit commit, List<AiEvidenceDraft> draft, String evidenceHash, String idempotency, String revision, AiModelProvider provider) {
		...
		AiAnalysisProviderDecision decision = new AiAnalysisProviderDecision(); decision.setAnalysisRun(run); decision.setProviderRole(AiProviderRole.PRIMARY); decision.setProviderKey(provider.providerKey()); decision.setProviderConfigHash(provider.providerConfigHash()); decision.setModelId(provider.modelId()); decision.setRoute(AiProviderRoute.NORMAL); decision.setStatus(AiProviderDecisionStatus.PENDING); decisions.save(decision); enqueueAfterCommit(run.getId()); return new Submission(run, true);
	}
