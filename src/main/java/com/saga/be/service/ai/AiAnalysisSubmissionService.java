...
public class AiAnalysisSubmissionService {
	...
	private Submission submit(UUID projectId, UUID gitCommitId, AiInvocationOrigin origin) {
		...
		try { return Objects.requireNonNull(transactions.execute(status -> persist(commit, draft, evidenceHash, idempotency, revision, provider, resolution, origin))); }
	...
	private Submission persist(GitCommit commit, List<AiEvidenceDraft> draft, String evidenceHash, String idempotency, String revision, AiModelProvider provider, AiCredentialResolver.Resolution resolution, AiInvocationOrigin origin) {
		AiAnalysisRun existing = AiRetryLineage.effective(runs, idempotency); if (existing != null && !AiRetryLineage.createsRetry(existing, origin)) { if (AiRetryLineage.shouldEnqueueExisting(existing)) enqueueAfterCommit(existing.getId()); return new Submission(existing, false); }
