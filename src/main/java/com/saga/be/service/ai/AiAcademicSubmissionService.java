...
public class AiAcademicSubmissionService {
 ...
 public AiAnalysisRun submitTask(UUID user, UUID projectId, UUID taskId) {
  ...
  return Objects.requireNonNull(tx.execute(status->persist(task.getProject(),AiArtifactType.TASK,taskId,snapshot.hash(),draft,evidenceHash,key,provider,resolution)));
 }
 ...
 public AiAnalysisRun submitCommit(UUID user, UUID projectId, UUID gitCommitId) {
  ...
  return Objects.requireNonNull(tx.execute(status->persist(commit.getRepo().getProject(),AiArtifactType.COMMIT,gitCommitId,sha,draft,evidenceHash,key,provider,resolution)));
 }
 ...
 private String idempotency(UUID projectId,AiArtifactType type,UUID artifactId,String revision,String evidenceHash,AiModelProvider provider,AiCredentialResolver.Resolution resolution) { return AiHashes.sha256(String.join("|",projectId.toString(),type.name(),artifactId.toString(),revision,AiAnalysisType.ACADEMIC_CLASSIFICATION.name(),evidenceHash,POLICY,PROMPT,SCHEMA,TAXONOMY,provider.providerConfigHash(),resolution.outcome().name(),String.valueOf(resolution.credentialFingerprint()))); }
 private AiAnalysisRun persist(com.saga.be.entity.project.Project project,AiArtifactType artifactType,UUID artifactId,String revision,List<AiEvidenceDraft> draft,String evidenceHash,String key,AiModelProvider provider,AiCredentialResolver.Resolution resolution) {
