...
public class AiAcademicSubmissionService {
 ...
 public AiAnalysisRun submitTask(UUID user, UUID projectId, UUID taskId) {
  ...
  List<AiEvidenceDraft> draft=new ArrayList<>(); draft.add(new AiEvidenceDraft(AiEvidenceType.TASK_FIELD,"task:"+taskId,snapshot.payloadJson(),null)); draft.addAll(candidates.build(academic));
 ...
 public AiAnalysisRun submitCommit(UUID user, UUID projectId, UUID gitCommitId) {
  ...
  AiAcademicContextService.Context academic=context.resolve(projectId); draft.addAll(candidates.build(academic));
