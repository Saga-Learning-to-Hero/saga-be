...
public class AiAcademicSubmissionService {
 ...
 public Submission submitTask(UUID user, UUID projectId, UUID taskId) {
  ...
  var resolution=requireCredential(task.getProject());
  ...
  String evidenceHash=hashEvidence(draft); String key=idempotency(projectId,AiArtifactType.TASK,taskId,snapshot.hash(),evidenceHash,provider,resolution);
  try{return Objects.requireNonNull(tx.execute(status->persist(task.getProject(),AiArtifactType.TASK,taskId,snapshot.hash(),draft,evidenceHash,key,provider,resolution)));}catch(AiRetryLineage.RetryAttemptConflict ex){return concurrent(ex);}
 }

 /** GitHub HTTP is deliberately complete before {@link TransactionTemplate#execute}. */
 public Submission submitCommit(UUID user, UUID projectId, UUID gitCommitId) {
  auth.requireReader(user,projectId);
  ...
  String evidenceHash=hashEvidence(draft); String key=idempotency(projectId,AiArtifactType.COMMIT,gitCommitId,sha,evidenceHash,provider,resolution);
  try{return Objects.requireNonNull(tx.execute(status->persist(commit.getRepo().getProject(),AiArtifactType.COMMIT,gitCommitId,sha,draft,evidenceHash,key,provider,resolution)));}catch(AiRetryLineage.RetryAttemptConflict ex){return concurrent(ex);}
 }

 /** ACADEMIC_CLASSIFICATION prefers a COURSE credential and only falls back to PLATFORM for a
 ...
 private String idempotency(UUID projectId,AiArtifactType type,UUID artifactId,String revision,String evidenceHash,AiModelProvider provider,AiCredentialResolver.Resolution resolution) { return AiHashes.sha256(String.join("|",projectId.toString(),type.name(),artifactId.toString(),revision,AiAnalysisType.ACADEMIC_CLASSIFICATION.name(),evidenceHash,POLICY,PROMPT,SCHEMA,TAXONOMY,provider.providerConfigHash(),resolution.outcome().name(),String.valueOf(resolution.identityFingerprint()))); }
 private Submission persist(com.saga.be.entity.project.Project project,AiArtifactType artifactType,UUID artifactId,String revision,List<AiEvidenceDraft> draft,String evidenceHash,String key,AiModelProvider provider,AiCredentialResolver.Resolution resolution) {
  AiAnalysisRun old=AiRetryLineage.effective(runs,key); if(old!=null&&!AiRetryLineage.createsRetry(old,AiInvocationOrigin.USER_REQUEST)){if(AiRetryLineage.shouldEnqueueExisting(old))after(old.getId());return new Submission(old,false);}
  int retryAttempt=AiRetryLineage.nextAttempt(old);
  ...
  AiRetryLineage.initialize(run,key,retryAttempt);
 ...
 private Submission concurrent(AiRetryLineage.RetryAttemptConflict ex) { AiAnalysisRun run=ex.winner(runs);if(AiRetryLineage.shouldEnqueueExisting(run))after(run.getId());return new Submission(run,false); }
