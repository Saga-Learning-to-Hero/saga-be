...
| GET | `/api/student/courses/{courseId}/dashboard` | Session | STUDENT | Student Dashboard Phase A+B1+B2+D1 (`weeklyCommits` last 3 ISO weeks; `actionableAlerts` = MSR_ANOMALY + PEER_REVIEW_PENDING only; no `contribution` / `GHOSTING_WARNING`) | `StudentDashboardController` |
