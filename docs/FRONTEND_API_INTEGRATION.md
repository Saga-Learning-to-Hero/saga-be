...
| GET | `/api/student/courses/{courseId}/dashboard` | Session | STUDENT | Student Dashboard Phase A+B1+B2 (`weeklyCommits` last 3 ISO weeks, `committedAt` only; chưa có `contribution` / `actionableAlerts`) | `StudentDashboardController` |
