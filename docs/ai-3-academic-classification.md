...
When `saga.ai.primary-provider=remote`, execution invokes saga-ai through `RemoteAiModelProvider`: SAGA BE remains the lifecycle, validation, persistence, and review authority; saga-ai performs inference only. Configure `SAGA_AI_RUNTIME_ENABLED`, `SAGA_AI_RUNTIME_BASE_URL`, and `SAGA_AI_RUNTIME_INTERNAL_TOKEN`. The call uses internal bearer authentication, has no automatic retry and no direct OpenAI fallback. Same-project Railway private networking is planned separately; the direct OpenAI adapter is retained temporarily for migration safety.
